...
def generate_content(info):
    ...
    # 天干解析 (壬水视角)
    ten_god = SHEN_DICT["壬"].get(tg, "运势")
    if tg in ["甲", "乙", "丙", "丁"]:
        tg_desc = f"代表心情明朗，才华得以施展或有财气。({ten_god})"
    elif tg in ["庚", "辛", "壬", "癸"]:
        tg_desc = f"代表压力较大，容易想太多或有竞争。({ten_god})"
    else:
        tg_desc = f"代表平稳，注重责任与规矩。({ten_god})"
        
    # 地支解析
    if dz == "申":
        dz_desc = "申金当令，印星过旺，身体容易感到沉重或疲惫。"
        conflict = "今日地支申金，与命局构成【寅申冲】，注意情绪波动和筋骨受伤！"
        weather = "今天就像阴雨天没带伞，湿漉漉的很难受。"
        summary = "需要躺平"
    elif dz == "寅":
        dz_desc = "寅木食神，本是好事，但要注意原局的冲动。"
        conflict = "今日地支寅木，再次引发【寅申冲】，才华虽有但易受压制。"
        weather = "风起云涌，内心有很多想法在碰撞。"
        summary = "情绪极易崩溃"
    elif dz == "巳":
        dz_desc = "巳火财星，但与申构成刑。"
        conflict = "今日地支巳火，构成【寅巳申三刑】，容易纠结、感到无恩之情。"
        weather = "雾里看花，看得到机会但过程纠结。"
        summary = "适合断舍离"
    elif dz == "午":
        dz_desc = "午火财星，内心渴望钱财。"
        conflict = "今日地支午火，引发【午午自刑】，容易内耗、想花钱。"
        weather = "烈日当空，内心燥热，容易冲动消费。"
        summary = "适合搞钱/买买买"
    elif dz == "亥":
        dz_desc = "亥水比劫，有人陪伴。"
        conflict = "今日地支亥水，与寅【寅亥合】，有人帮、感觉舒服。"
        weather = "春雨润物，有人理解你的不容易。"
        summary = "适合约会/谈事"
    else:
        dz_desc = "地支平稳，按部就班。"
        conflict = "今日地支与命局无剧烈冲突，平稳度过。"
        weather = "多云转晴，心情逐渐平复。"
        summary = "适合搞钱"
    ...
    if tg in ["庚", "辛", "壬", "癸"] or dz in ["申", "酉", "亥", "子"]:
        ...
        advices.append({"title": "去吃顿好的奖励自己", "logic": "喜用神到位，享受生活。"})
        advices.append({"title": "记录下今天的灵感", "logic": "食伤生财，灵感就是财富。"})
